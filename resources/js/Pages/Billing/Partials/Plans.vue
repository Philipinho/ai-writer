            ...
            <div v-for="plan in plans" :key="plan.id"
                ...
                <p class="mt-6 flex items-baseline gap-x-1">
                        <span
                            ...
                            <span v-if="frequency.value === 'monthly'">{{ plan.monthly_amount }}</span>
                            <span v-if="frequency.value === 'yearly'">{{ plan.yearly_amount }}</span>
                        </span>
                ...
                <ul role="list"
                    :class="[plan.featured ? 'text-gray-300' : 'text-gray-600', 'mt-8 space-y-3 text-sm leading-6 xl:mt-10']">
                    <li v-for="feature in plan.features" :key="feature" class="flex gap-x-3">
                        <CheckCircleIcon
                            :class="[plan.featured ? 'text-white' : 'text-indigo-600', 'h-6 w-5 flex-none']"
                            aria-hidden="true"/>
                        {{ feature }}
                    </li>
                </ul>
...
export default {
    ...
    methods: {
        subscribe(plan, frequency) {
            const priceId = frequency.value === 'monthly' ? plan.stripe_monthly_price_id : plan.stripe_yearly_price_id;

            axios.post(route('checkout'), {plan: plan.name, price_id: priceId})
                .then(response => {
